  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const accRes = await fetch(`http://127.0.0.1:8000/accounts`);
        const accData = await accRes.json();
        const found = accData.find((a: Account) => a.id === accountIdNum);
        setAccount(found);

        // Fetch all transactions for this account
        const txRes = await fetch(`http://127.0.0.1:8000/transactions?account_id=${accountIdNum}&party_id=${partyIdNum}`);
